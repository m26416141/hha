...
$getArticleId = $_GET['servicesid'];
$sql = "SELECT * FROM services WHERE services_id = $getArticleId";
$query_run = mysqli_query($con, $sql);
while ($row = mysqli_fetch_array($query_run)) {
    $title = $row['services_title'];
    // $getDate = date_create($row['article_date']);
    // $date = date_format($getDate, "d/m/Y");
    $desc = $row['services_content'];
    ...
    <head>
        ...
        <title>Edit Services - Howard Hermes</title>
            ...
            <form method="POST" enctype="multipart/form-data">
                <div class="row header">
                    <div class="col-sm d-flex align-items-center" style="padding: 0px; display: flex">
                        <h4 style="font-weight: 700; margin: 30px">Edit Services / ' . $services . '</h4>
                        <span id="editS"></span>
                    </div>
                    ...
                    <div class="col-sm-4">
                        <div class="col-sm-12">
                            <div class="form-add-careers">
                                ...
                                <input class="form-input" type="text" name="title" id="inputTitle" placeholder="Services title" value="' . $title . '">
                                <label for="inputMedia">Select media</label>
                                <select class="btn btn-secondary btn-block" name="media" id="inputMedia" style="margin-bottom: 20px; font-size: 17px;">
                                    <option value="0">-</option>
                                    <option value="1">Image</option>
                                    <option value="2">Video</option>
                                </select>
                                <label for="inputVideo">Video link</label>
                                <input class="form-input" type="text" name="video" id="inputVideo" placeholder="Video Link" value="' . $img . '">
                                ...
                                <p style="border: 1px solid red; padding: 20px">You can leave "upload image" empty if you want to use video</p>
                            </div>
                        </div>
                    </div>
...
if (isset($_POST["submit"])) {
    ...
    if ($arMedia != 0) {
        if (in_array($fileExt, $allowedFile)) {
            if ($fileError === 0) {
                if ($fileSize < 500000) {
                    ...
                    if (move_uploaded_file($fileTmp, '../assets/uploads/' . $fileNameNew) === true) {
                        $sql = "UPDATE services SET services_title = '$arTitle', services_img = '$fileNameNew', services_content = '$arDesc', services_img_status = '$arMedia' WHERE services_id = '$getArticleId'";
                        if ($con->query($sql) === true) {
            ...
            if ($arMedia == 2) {
                $sql = "UPDATE services set services_title = '$arTitle', services_content = '$arDesc', services_img = '$arVideo', services_img_status = '$arMedia' WHERE services_id = '$getArticleId'";
                if ($con->query($sql) === true) {
                    // header("Location: edit_article.php?editarticlesuccess");
                    echo "
                    <script>
        ...
        <script>
            document.getElementById('editS').style.visibility = 'visible';
            document.getElementById('editS').style.backgroundColor = '#dc3545';
            document.getElementById('editS').innerHTML = 'Please select media type';
        </script>";
    }
}
